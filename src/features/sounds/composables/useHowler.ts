...
export const useHowler = () => {
  ...
  const handleFirstUserGesture = async () => {
    ...
    try {
      ...
      // Some mobile browsers expose no resumable AudioContext.
    }
  ...
  onMounted(() => {
    ...
    // Mobile browsers require a real user gesture before AudioContext
    // and Howler can start playback. Listen once from the global page so
    // the existing music/sound toggle can unlock audio on first interaction.
